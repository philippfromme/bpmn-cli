...
const primitiveTypes = new Set(["Boolean", "Integer", "Real", "String"]);
const presentationPrefixes = new Set([
  "bpmndi",
  "dc",
  "di",
  "xml",
...
function prefixOf(type, fallback) {
  return type.includes(":") ? type.slice(0, type.indexOf(":")) : fallback;
}

function classifyDescriptorProperty(descriptor, property) {
  if (
    presentationPrefixes.has(descriptor.prefix) ||
    presentationPrefixes.has(prefixOf(property.type, descriptor.prefix))
  ) {
    return "presentation";
  }
  ...
  if (property.isAttr || primitiveTypes.has(property.type)) {
    return "scalar";
  }

  return "contained-child";
}

function propertyClassificationEntries() {
  return descriptorSources
    .flatMap(({ descriptor }) =>
      descriptor.types.flatMap((type) =>
        (type.properties ?? []).map((property) => [
          `${elementTypeName(descriptor, type)}#${property.name}`,
          classifyDescriptorProperty(descriptor, property)
        ])
      )
    )
...
function containedChildPropertiesByType() {
  const types = new Map(
    descriptorSources.flatMap(({ descriptor }) =>
      descriptor.types.map((type) => [
        elementTypeName(descriptor, type),
        { descriptor, type }
      ])
    )
  );

  function propertiesOf(typeName, visited = new Set()) {
    ...
    const parents = (entry.type.superClass ?? []).flatMap((parent) =>
      propertiesOf(parent.includes(":") ? parent : `${entry.descriptor.prefix}:${parent}`, visited)
    );
    return [...parents, ...(entry.type.properties ?? [])];
  }
  ...
  return descriptorSources
    .flatMap(({ descriptor }) =>
      descriptor.types
        ...
        .map((type) => {
          const properties = propertiesOf(elementTypeName(descriptor, type));
          const names = [...new Set(
            properties
              .filter(
                (property) =>
                  classifyDescriptorProperty(descriptor, property) === "contained-child"
              )
              .map((property) => JSON.stringify(property.name))
          )].sort();
          return [elementTypeName(descriptor, type), names];
        })
    )
    .sort(([left], [right]) => left.localeCompare(right));
}
...
function adapterContents() {
  ...
  const traitProperties = [...traitExtensions]
    ...
    .join("\n");
  const classifications = propertyClassificationEntries()
    .map(
      ([key, classification]) =>
        `  ${JSON.stringify(key)}: ${JSON.stringify(classification)},`
    )
    .join("\n");
  const containedChildProperties = containedChildPropertiesByType()
    .map(
      ([type, properties]) =>
        `  ${JSON.stringify(type)}: ${properties.length ? properties.join(" | ") : "never"};`
    )
    .join("\n");
